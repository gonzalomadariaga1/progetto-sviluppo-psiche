...
<main id="main">
  ...
  padding-top: 100px;" >
    {{ csrf_field() }}
    <section class="why-us section-bg"  style="padding: 20px !important;">
              ...
              <div id="stepper1" class="bs-stepper linear">
                <div class="bs-stepper-header">
                  <div class="step" data-target="#test-l-1">
                    <button type="button" class="btn step-trigger">
                      ...
                      <span class="bs-stepper-label" style="font-size: 15px;">{{__('reservar.service')}}  </span>
                    </button>
                  </div>
                ...
                <div class="bs-stepper-content">
                      ...
                      <div class="card-body fondo-crema" style="border:none;">
                        <div class="row">
                          ...
                          <div class="col-lg-4 col-xs-12">
                            <select  id="servicio_id" class="form-control selectpicker" title="{{__('reservar.selectServicio')}}"  >
                              @foreach($servicios as $servicios)
                              <option style="color: black" value="{{$servicios->id}}_{{ $servicios->{'name_'.app()->getLocale()} }}">{{ $servicios->{'name_'.app()->getLocale()} }}</option>
                              @endforeach
                            </select>
                          </div>
                          <div class="col-lg-4 col-xs-12">
                            <select  id="modalidad_id" class="form-control selectpicker" title="{{__('reservar.selectModalidad')}}" >
                              @foreach($modalidades as $modalidad)
                              <option value="{{$modalidad->id}}_{{ $modalidad->{'name_'.app()->getLocale()} }}_{{$modalidad->precio}}_{{$modalidad->duracion}}">{{ $modalidad->{'name_'.app()->getLocale()} }}</option>
                              @endforeach
                            </select>
                          </div>
                          <div class="col-lg-4 col-xs-12">
                        ...
                        {{-- <div class="row">
                          ...
                          <div class="col-lg-8 col-xs-12">
                            ...
                            <table class="table" id="tablaresumen">
                              <tbody>
                                <tr>
                                  <th scope="row">{{__('reservar.service')}}</th>
                                  <td id="row-servicio"></td>
                                  
                                </tr>
                                <tr>
                                  <th scope="row">{{__('reservar.modalidad')}}</th>
                                  <td id="row-modalidad"></td>
                                  
                                </tr>
                                <tr>
                                  <th scope="row">{{__('reservar.duracion')}}</th>
                                  <td id="row-duracion"></td>
                                  
                                </tr>
                                <tr>
                                  <th scope="row">{{__('reservar.valor')}}</th>
                                  <td id="row-valor"></td>
                  ...
                  {{-- <div id="test-l-2" class="content">
                    ...
                    <button type="button" class="btn btn-primary" onclick="stepper1.previous()">{{__('reservar.back')}}</button>
                    <button type="button" class="btn btn-primary" onclick="paso2_to_paso3()">{{__('reservar.next')}}</button>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            {{-- <a class="btn btn-lg crema animate__animated animate__fadeInUp mt-3" href="/reservar" role="button">{{__('inicio.textobotonbajocarr')}}</a> --}}
            <button type="button" class="btn btn-secondary"  data-bs-dismiss="modal" >{{__('reservar.cancelar')}}</button>
            {{-- <a href=" {{route('reservar')}} " class="btn btn-secondary" role="button">{{__('reservar.cancelar')}}</a> --}}
...
<script>
  ...
  function paso1_to_paso2(){
    ...
    var value_modalidad = $("#modalidad_id :selected").val();

    if ( value_servicio == ''){
      Swal.fire({
            icon: 'error',
            title: '{{__('reservar.error_seleccione_servicio')}}',
      ...
      Swal.fire({
            icon: 'error',
            title: '{{__('reservar.error_seleccione_modalidad')}}',
            confirmButtonText: 'OK',
            footer: '',
            showCloseButton: true,
            timer: 5000
        })
    } else if ( value_especialista == '' ) {
      Swal.fire({
            icon: 'error',
            title: '{{__('reservar.error_seleccione_especialista')}}',
            confirmButtonText: 'OK',
            footer: '',
            showCloseButton: true,
            timer: 5000
      ...
      var value_modalidad_sep = value_modalidad.split('_');
    var modalidad_id = value_modalidad_sep[0];
    ...
    var $hora; 
    jQuery.ajax({
        url: '/reservar/'+value_especialista+'/'+modalidad_id+'/get_fechas/',
        type: "GET",
        dataType: "json",
        async:false,
        error:function(e){
        ...
        success:function(respuesta)
        {
          ...
          if ( respuesta.length === 0 ){
            ...
            onChangeDateTime:function(dp,$input){
              ...
              jQuery.ajax({
                  url: '/reservar/'+$hora+'/'+modalidad_id+'/get_horas/',
                  type: "GET",
                  dataType: "json",
                  async:false,
                  error:function(e){
  ...
  function datosModal(){
    ...
    datoFecha = document.getElementById('datetimepicker3').value;
    //console.log("datofehca",datoFecha);
    
    datohora = document.getElementById('datetimepicker5').value;
    datohora_fin = document.getElementById('input_hora_fin').value;
    datohora_only = datohora.slice(-5)
    datosModalidad = document.getElementById('modalidad_id').value.split('_');
    datosServicio = document.getElementById('servicio_id').value.split('_');
    
    var euro = ' euros.'
    var min = ' min.'
    
    document.getElementById("row-servicio-modal").innerHTML = datosServicio[1];
    $('#input_servicio_id').val(datosServicio[0]);
    $('#input_servicio').val(datosServicio[1]);
    document.getElementById("row-modalidad-modal").innerHTML = datosModalidad[1];
    $('#input_modalidad_id').val(datosModalidad[0]);
    $('#input_modalidad').val(datosModalidad[1]);
    document.getElementById("row-valor-modal").innerHTML = datosModalidad[2]+euro;
    document.getElementById("row-totalvalor-modal").innerHTML = datosModalidad[2]+euro;
    $('#input_valor').val(datosModalidad[2]);
    document.getElementById("row-duracion-modal").innerHTML = datosModalidad[3]+min;
    ...
    //console.log("tetetee",codigopais+' '+telefono);
    ...
    jQuery.ajax({
                  ...
                  success:function(respuesta)
                  {
                    ...
                    if ( respuesta == 0 ){
                      ...
                      var porcentaje = ' %.'
                      var euros = ' euros.'
                      var valor_original = datosModalidad[2];
